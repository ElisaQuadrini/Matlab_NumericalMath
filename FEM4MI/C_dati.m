...
if strcmp(test, 'Test1') %se vale Test1 allora faccio operazioni
Dati = struct( 'name',             test,...
               ... % Test name
               'domain',           [-1,1],...                          
               ... % Domain bounds = dominio      
               'mu',               0.25, ...
               ... % Diffusive term ...
               'beta',             0, ...
               ... % Advective term ... = problemi differenziali più avanzati
               'force',            '1-3*x.^2 ',...  % essendo 3 scalare, . non serve
               ... % Forcing term = >f(x), qua come stringa
               'exact_sol',        '(1+x).^2.*(1-x).^2 ',...      
               ... % Definition of exact solution = soluzione esatta
               'grad_exact',       '2*(1+x).*(1-x).^2-2*(1+x).^2.*(1-x) ',...    
               ... % Definition of exact gradient (x comp) (della soluzione esatta)
               'fem',              'P1',...         
               ... % P1-fe
               'nqn_1D',            2,...           
...
elseif strcmp(test, 'Es2') %se vale Test1 allora faccio operazioni
Dati = struct( 'name',             test,...
               ... % Test name
               'domain',           [-1,1],...                          
               ... % Domain bounds = dominio      
               ...
               'beta',             0, ...
               ... % Advective term ... = problemi differenziali più avanzati
               'force',            '-sign(x+0.1)./sqrt(abs(x+0.1)) ',...  
               ... % Forcing term = >f(x), qua come stringa
               'exact_sol',        '(x+0.1).* sqrt(abs(x+0.1))',...      
               ... % Definition of exact solution = soluzione esatta
               'grad_exact',       'sqrt(abs(x+0.1))+0.5*(x+0.1)./sqrt(abs(x+0.1)).*sign(x+0.1)',...    
               ... % Definition of exact gradient (x comp) (della soluzione esatta)
               'fem',              'P1',...         
               ... % P1-fe
               'nqn_1D',            2,...           
